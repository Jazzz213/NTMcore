            ...
            player.sendMessage(ChatColor.GREEN + "Ожидайте, скоро вам ответит администратор");
            {
                for (Player p1 : Bukkit.getOnlinePlayers()) {
                    if (p1.hasPermission("group.moderator")) {
                        p1.spigot().sendMessage(ChatUtil.genChatMessage("[R]", player.getName(), message, "Репорт", "Написать " + player.getName(), null, new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/tell " + player.getName() + " "), net.md_5.bungee.api.ChatColor.AQUA, net.md_5.bungee.api.ChatColor.of("#9EFF86"), net.md_5.bungee.api.ChatColor.WHITE));
