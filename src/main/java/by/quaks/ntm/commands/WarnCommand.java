...
public class WarnCommand implements CommandExecutor, TabExecutor {
    @Override
            ...
            default:{
                if(Arrays.asList(Bukkit.getOfflinePlayers()).contains(Bukkit.getOfflinePlayer(args[0]))){
                    ...
                    if(WarnList.get().get(name+".warns."+amount)==null){
                        ...
                        sender.getServer().dispatchCommand(sender.getServer().getConsoleSender(), "discordsrv broadcast " + asg[0] + " получил варн мощностью "+asg[1]+" ("+removeChars(message)+")");
                    }
